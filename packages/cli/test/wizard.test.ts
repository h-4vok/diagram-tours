...
describe("runWizard", () => {
  ...
  it("recovers from invalid menu input and path input", async () => {
    const io = createIo(["9", "3", "./missing.tour.yaml", "./examples/flowchart/checkout-payment-flow.mmd", "y", "0.0.0.0", "9000"]);
  ...
  it("accepts a direct diagram file path from the wizard", async () => {
    const io = createIo(["3", "./examples/flowchart/checkout-payment-flow.mmd", "n", "", ""]);
